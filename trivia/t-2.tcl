...
set t2(A01) {%1 Gathering data in %x seconds, get ready!!! }
...
proc TOnOff {nk uh hn ch tx {from 0} } {  global t2 tclr botnick nick
 ...
 if {$t2(-ison)=="0"} {
   ...
   } else {  putquick "PRIVMSG $ch :4Trivia1 started.. 2Loading..."  }
   set t2(-htime) $t2(qtime)  ;  set t2(-ptime) $t2(ptime)  ;  set t2(-stime) $t2(stime)
   ...
   putquick "PRIVMSG $ch : Trivia is 4stopped1 after # of questions not answered. To play again, type !start . Have fun."
 }
}
 ...
 if {$t2(-hntnum)>=$t2(hntcnt) || $t2(-hntnum)=="-1" || $t2(-hntnum)=="-2"} {  set istop 0
   if {$t2(-hntnum)=="-1"} {
     ...
     if {$iskaos=="1"} {  set kleft [expr {[llength $t2(-kansls)]/4}]
       ...
       if {$t2(givkaos)!="0"} {  set ktutmp "$tclr(-ktu)$tclr(-ktu2)2(13KAOS2)2 Words remained: $tclr(-kng)"
         foreach {kans hn2 hn3 kvar} $t2(-kansls) {  append ktutmp "\[$kans\] "  }
       } else {  set ktutmp "$tclr(-ktu2) Times up! "  }
       putquick "PRIVMSG $t2(chan) : 2(13KAOS2)2 question ended."
       ...
       if {$t2(givansr)!="0"} { set ansr "$tclr(-tu2) The answer is \[ $tclr(-tu1) $t2(-answer) $tclr(-tu2) \] " }
       putquick "PRIVMSG $t2(chan) : Looks like nobody knows, $tclr(-tu1)Time's up! $ansr"
       putquick "PRIVMSG $t2(chan) : Current round is over. Good luck on the next one.."
     }
   }
  ...
  if {$iskaos=="1"} {  set sec $t2(ktime)  ;  set doshort 0
    ...
    set t2(-hnt2) "$tclr(-khnt) Hint:$tclr(-khnt2)"
    ...
    set t2(-hnt2) "$tclr(-hint) Hint:$tclr(-hint2) $t2(-hnt2) $t2(-endclr) "
  ...
  if {$iskaos=="1"} {  set sec $t2(ktime)  ;  set doshort 0
    ...
    set t2(-hnt3) "$tclr(-khnt) Hint:$tclr(-khnt2)"
    ...
    set t2(-hnt3) "$tclr(-hint) Hint:$tclr(-hint2) $t2(-hnt3) $t2(-endclr) "
    if {$t2(descend)=="1"} {
...
proc TMkLines {} {  global t2 tclr botnick nick ;  set t2(-otimer) "" ;  set doques 1 ;  set isbon 0
 ...
 if {$qfline ne ""} {  set qls [split $qfline *] ;  set t2(-allansls) [TStrLo [lrange $qls 1 end]]
  ...
  if {$doques=="1"} {
    ...
    set t2(-hint) "$tclr(-hint) Hint:$tclr(-hint2) $t2(-hint) $t2(-endclr) "
    if {$pnttmp=="1"} {  set point Point  } else {  set point Points  }
    if {$isbon=="1"} {
      append t2(-hint) "$tclr(-bonus) 4B07O03N12U06S Question Value :$tclr(-bon2) $pnttmp $point "
    } else {  append t2(-hint) "$tclr(-pnt1) Question Value :$tclr(-pnt2) $pnttmp $point "  }
    set t2(-points) $pnttmp
  } else {
    set t2(-hint) "$tclr(-khnt) Hint:$tclr(-khnt2) [string trim $t2(-hint)] $t2(-endclr) "
    ...
    if {$t2(kstyle)=="0" && $doshort=="0"} {
      ...
      set t2(-hint) "$tclr(-khnt)H1nt:$tclr(-khnt2) [TDoNum $kanscnt] Possible Answers $t2(-endclr) "
