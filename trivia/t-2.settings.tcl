...
set t2(chan) "#cebu"   ;# channel to run bogustrivia in  <- !! YOU MUST SET THIS !! #

set t2(mflag) "m|m"   ;# flags for /msg commands  (.add .mix) #

set t2(on) "!start"      ;# public trigger to turn game on #
set t2(off) "!stop"        ;# public trigger, game off  ("" = same as on trigger) #
set t2(oflag) ""   ;# flags to turn the game on ("" = everyone) #
set t2(sflag) "m|m"      ;# flags to turn game off    ("" = same as on flags) #
...
set t2(ptime) "15"    ;# play time between questions (6 to 60 seconds) #

set t2(dobonus) "1"      ;# how often to do a bonus (by question count) (0=off) #

set t2(minbonus) "10000"  ;# min points for bonus (empty (or 0)=uses hpoint setting as before) #  <- NEW SETTING #
set t2(maxbonus) "15000"  ;# max points for bonus (above minbonus (or hpoint) #

set t2(kaos) "5"     ;# how often to do a kaos (by question count) (0=off) (1=do only kaos) #
set t2(ktime) "15"    ;# time between hints on kaos (6 to 90 seconds) #
...
set t2(randad) "5"     ;# how often to show random ad (by question count) (0=off) #
...
#CEBU Trivia Statistics at: https://bit.ly/cebu-trivia
TexTwist Game Commands: .textwist (to start the game), .stop (to stop the game), .twist (twist the words), .list (show all the words list), .top (show top stats), .score (show score)
Bingo Game Commands: !play 1 2 3 4 5, !bingostop (to stop the game), !bingo (open a bingo game in the channel (ADMIN)), !start (start bingo game in the channel (ADMIN))
To check nick stats in the channel: Please type, !stat, !stat <nick>
Useful commands: !geo <ip>, !time <show to GMT +8>, !omdb <movie title>, .i <nick> (show nick idle in channel), !seen <nick> | !seen top | !seen stat, !lag
Duck Hunt commands: !help, !bang, !reload, !shop, !topduck, !duckstats, !lastduck
Scrabble Game: !scrabble - start game, !scrabble stop - stop the game, !top <general> / <round>, !won <user> - view user statistics, !game - show the current letters from which you will form words.
Welcome to Our Channel, kick back and play some Trivia!!
Think a Q&A is incorrect? It can't be the bot is always right. hehe
Show a Hint, q Will Repeat the Current Question
...
set t2(sqcnt) "2"     ;# use slow-times or idle-stop after this many questions not played #
set t2(qslow) "6"    ;# slow time between hints (6 to 90 seconds) (0 = idle-stop the game) #
set t2(pslow) "6"    ;# slow time between questions (6 to 90 seconds) #
...
# the times between hints & questions increase to these settings, #
# or idle-stop the bogustrivia game. #
set t2(rqcnt) "2"   ;# use resting-times or idle-stop after this many not played #
set t2(rest) "0"    ;# resting time between hints/questions (6 to 120) (0 = idle-stop the game) #
...
set t2(oldusr) "54000"  ;# remove all users not seen in this many days ?? (0=no) #
...
set t2(p-pwww) "https://bit.ly/4bZmYjE"   ;# web address of the stats pages #
...
set tscl(gr1) "01"    ;# code:  %1  in greeting text settings above #
set tscl(gr2) "01"    ;# code:  %2  in greeting text settings above #
set tscl(gstat) "05"
set tscl(gsta2) "01"
set tscl(gsta3) "05"
